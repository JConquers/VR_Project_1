...
- **Testing on external images** The model gave a high accuracy for the images in the test dataset. However, when we checked the model on images where there was a full body view of the person and not just the face, there were inaccuracies. This can be seen in the part_a_b.ipynb file in the scripts folder.
